...
int main(int argc, char **argv) 
{
    ...
    ros::init(argc, argv, "cssgra_action_server", ros::init_options::AnonymousName);
    ...
    Server server(*n, "follow_joint_trajectory_controller/follow_joint_trajectory", boost::bind(&on_goal, _1, &server), false);
...
void on_goal(const control_msgs::FollowJointTrajectoryGoalConstPtr& goal, Server* as){
    ...
    for (int i = 0; i < PointsSize; i++)
    {
        ...
        {
            set_position_client = n->serviceClient<webots_ros::set_float>(string("/cssgra/") + string(armNames[j]) + string("/set_position"));   
        ...
        ROS_INFO("position set:%f, %f, %f, %f, %f", armpositions[0], armpositions[1], armpositions[2], armpositions[3], armpositions[4]);
